...
public class PhotoPreviewActivity_2 extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        maxSize = BaseAppUtil.getDeviceWidth(mContext)
                        * BaseAppUtil.getDeviceHeight(mContext);
        setupUI();
    }
...
//                builder.into(photoView);
                ...
                int[] size = BaseBitmapUtil.getBitmapWidthAndHeight(mf.getRealPath());
                if(size[0]*size[1] > maxSize){
                    ...
                    Glide.with(mContext)
                            ...
                            .dontAnimate()
                            .dontTransform()
                            .override(BaseAppUtil.getDeviceWidth(mContext) > size[0] ? size[0] : BaseAppUtil.getDeviceWidth(mContext),
                                    BaseAppUtil.getDeviceHeight(mContext) > size[1] ? size[1] : BaseAppUtil.getDeviceHeight(mContext))
                            .into(photoView);
                }else{
                    Glide.with(mContext)
                            .load(new File(mf.getRealPath()))
